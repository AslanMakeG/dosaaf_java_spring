...
public interface NewsRepo extends CrudRepository<NewsEntity, Long> {
    @Query(value="SELECT n.* FROM news n ORDER BY creation_date_time asc OFFSET ?1 LIMIT ?2", nativeQuery = true)
    List<NewsEntity> findAllByDateASC(Integer page, Integer limit);
    @Query(value="SELECT n.* FROM news n ORDER BY creation_date_time desc OFFSET ?1 LIMIT ?2", nativeQuery = true)
